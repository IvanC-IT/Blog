...
public class ResponseEntityHandler {
	...
	private void initialize() {
		ApiResponseCustom arc = new ApiResponseCustom(Instant.now(), status.value(), status.getReasonPhrase(), 
				msg, request.getRequestURI());
		responseEntity = new ResponseEntity<ApiResponseCustom>(arc, status);
	}
